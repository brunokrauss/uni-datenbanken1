...
-- YOUR CODE HERE
	...
	THEN
		...
		SELECT stationcount INTO L_MAXAMOUNTSTATIONS FROM zone WHERE zoneid = IN_ZONE_ID ORDER BY zonepricereduced ASC LIMIT 1;
	ELSE
		SELECT zoneprice INTO L_PRICE FROM zone WHERE zoneid = IN_ZONE_ID ORDER BY zoneprice ASC LIMIT 1;
		SELECT stationcount INTO L_MAXAMOUNTSTATIONS FROM zone WHERE zoneid = IN_ZONE_ID ORDER BY zoneprice ASC LIMIT 1;
	END IF;
